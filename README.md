# Hi, I'm Pranav 👋
I get curious about how things work and end up somewhere I didn't expect.

I'm 18, based in Hyderabad. I build things, investigate systems, and write about what I find — in plain language.

Currently building MathLCM — a Large Concept Model for mathematical reasoning. AI that reasons at the concept level, not the token level. 78.2% accuracy on competition-level math. Built in two weeks for $15.

---

## 🛠️ What I work with
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## 🔬 Research
| Project | What it is |
|---|---|
| [MathLCM](https://huggingface.co/PranavWuppala/MathLCM) | Large Concept Model for math reasoning. Transformer predicts reasoning moves, SymPy executes with guaranteed correctness. 78.2% accuracy on 9-class step prediction. |

---

## 🔧 Tools Built
| Repo | What it does |
|---|---|
| [apk-scanner](https://github.com/PranavWuppala/apk-scanner) | Scans Android APKs for suspicious permissions, cross-references privacy policies |
| [cctv-scan](https://github.com/PranavWuppala/cctv-scan) | Maps publicly exposed CCTV devices using internet scanning APIs |
| [voice-cloning-pipeline](https://github.com/PranavWuppala/voice-cloning-pipeline) | Fish Audio S2Pro pipeline with audio preprocessing, YAML config, CLI |

---

## ✍️ Writing
| Article | About |
|---|---|
| [I Cloned a Voice in My Bedroom](https://medium.com/@PranavWuppala/i-cloned-a-voice-with-just-a-laptop-wi-fi-and-5-thats-the-problem-86035b2f1313) | Cloned a voice for $5 in under a day — here's why that's a problem |
| | [Somebody, Somewhere, Might Be Watching You](https://medium.com/@PranavWuppala/somebody-somewhere-might-be-watching-you-using-tech-you-paid-for-6d1e00174523) | How thousands of cameras are publicly exposed on the internet || How thousands of cameras are publicly exposed on the internet |

---

## 🔗 Find me
[![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)](https://huggingface.co/PranavWuppala)
[![Medium](https://img.shields.io/badge/Medium-000000?style=flat&logo=medium&logoColor=white)](https://medium.com/@PranavWuppala)
[![Substack](https://img.shields.io/badge/Substack-FF6719?style=flat&logo=substack&logoColor=white)](https://substack.com/@pranavwuppala)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/pranavwuppala)
[![X](https://img.shields.io/badge/X-000000?style=flat&logo=x&logoColor=white)](https://x.com/PranavWuppala)
